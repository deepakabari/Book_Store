 ...
 * @description Streams a file to the client if it exists, otherwise return nexts an error.
 */
export const viewFile: Controller = async (req, res, next) => {
    ...
    if (!fs.existsSync(filePath)) {
        ...
        return next(new ErrorHandler(httpCode.BAD_REQUEST, messageConstant.FILE_NOT_FOUND));
    } else {
 ...
 * @param next - The next middleware function in the application’s request-response cycle.
 * @description Sends a file for the client to download if it exists, otherwise return nexts an error.
 */
export const downloadFile: Controller = async (req, res, next) => {
    ...
    if (!fs.existsSync(filePath)) {
        ...
        res.download(filePath, fileName, (err) => {
            if (err) {
                return next(new ErrorHandler(httpCode.BAD_REQUEST, messageConstant.ERROR_DOWNLOAD_FILE));
            }
        });
    }
};
...
export const downloadFiles: Controller = async (req, res, next) => {
    ...
    if (!Array.isArray(fileNames) || !fileNames.length) {
        return next(new ErrorHandler(httpCode.BAD_REQUEST, messageConstant.NO_FILE_SELECTED));
    }
...
export const exportBooks: Controller = async (req, res, next) => {
    ...
    do {
        ...
        const response = await axios.get(`${linkConstant.EXPORT_BOOK}?page=${page}&pageSize=${pageSize}`, {
            headers: {
                Authorization: token,
            },
        });
