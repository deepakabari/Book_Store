...
export const createCustomer: Controller = async (req, res, next) => {
    try {
        ...
        if (!stripeCustomer) {
            ...
            // If the user already has a Stripe customer ID, throw an error
            throw new ErrorHandler(httpCode.BAD_REQUEST, messageConstant.CUSTOMER_EXISTS);
        }
...
export const addCardHolder: Controller = async (req, res, next) => {
    try {
        ...
        if (user.cardHolderId) {
            throw new ErrorHandler(httpCode.BAD_REQUEST, messageConstant.CARD_HOLDER_ALREADY_EXISTS);
        }
        ...
        if (!cardholder) {
            throw new ErrorHandler(httpCode.BAD_REQUEST, messageConstant.CARD_HOLDER_FAILED);
        }
...
export const authorization: Controller = async (req, res, next) => {
    try {
        ...
        if (cartItems.length === 0) {
            throw new ErrorHandler(httpCode.NOT_FOUND, messageConstant.CART_EMPTY);
        }
